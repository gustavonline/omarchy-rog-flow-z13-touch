...
BarWidget {
  id: root
  moduleName: "io.github.gustavonline.z13-touch-indicators"
  ...
  IpcHandler {
    target: "io.github.gustavonline.z13-touch-indicators"

    function refresh(): void {
      root.broadcast("refresh")
    }

    // Keep the touch toggle externally observable so its one-gesture state
    // transition can be regression-tested without synthesising pointer input.
    function touchRevealState(): string {
      return root.touchRevealPinned ? "shown" : "hidden"
    }

    function toggleTouchReveal(): string {
